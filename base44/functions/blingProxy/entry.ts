...
async function doRefreshToken(base44, tokenRecord) {
  const clientId = Deno.env.get('VITE_BLING_CLIENT_ID') || Deno.env.get('BLING_CLIENT_ID');
  const clientSecret = Deno.env.get('VITE_BLING_CLIENT_SECRET') || Deno.env.get('BLING_CLIENT_SECRET');
